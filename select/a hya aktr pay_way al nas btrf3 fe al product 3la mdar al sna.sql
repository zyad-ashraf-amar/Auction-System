SELECT pw.pay_way_name, EXTRACT(MONTH FROM rp.DATE_OF_ACTION) AS month,  COUNT(DISTINCT p.product_id) AS total_sales
FROM product p
...
GROUP BY pw.pay_way_name , EXTRACT(MONTH FROM rp.DATE_OF_ACTION)
ORDER BY  EXTRACT(MONTH FROM rp.DATE_OF_ACTION);